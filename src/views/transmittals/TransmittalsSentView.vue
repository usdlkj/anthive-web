...
async function load() {
  ...
  loading.value = true
  try {
    const res = await axiosInstance.get('/transmittals/sent', {
      params: { projectId: projectId.value },
    })
    rows.value = res.data?.data ?? []
  } catch (error: any) {
    rows.value = []
    toast.error(error?.response?.data?.message ?? 'Failed to load sent transmittals')
  } finally {
    loading.value = false
  }
}
...
<template>
  <PageWrapper title="Sent Transmittals" breadcrumb="Transmittals / Sent">
    <div
      v-if="!projectId"
      ...
      <table class="w-full text-[13px]">
        <thead class="bg-sidebar-panel text-nav-text">
          <tr class="text-left">
            ...
            <th class="px-3 py-2.5 font-semibold">Recipients</th>
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-3 py-2.5 text-body-text">{{ item.recipients?.length ?? 0 }}</td>
